...
class TestPlugin extends queue_1.QueuePlugin {
    constructor(queue) {
        super('test', queue);
        this.setInitialTaskStatus = (data) => 'pending';
        this.processTask = async (task) => { };
    }
}
(0, node_test_1.describe)('QueuePlugin', () => {
    (0, node_test_1.test)('kick should not call loop when syncInProgress is true', async () => {
        const queue = new queue_1.QueueManager();
    ...
    (0, node_test_1.test)('loop should not process tasks when syncInProgress is true', async () => {
        const queue = new queue_1.QueueManager();
        queue.getNextTaskFor = () => undefined;
        const plugin = new TestPlugin(queue);
        // Spy on processTask
    ...
    (0, node_test_1.test)('kick should call loop once syncInProgress is set to false', async () => {
        const queue = new queue_1.QueueManager();
    ...
    (0, node_test_1.test)('mock sync method should retry every 30 seconds until status is idle', async () => {
        const queue = new queue_1.QueueManager();
        queue.getNextTaskFor = () => undefined;
        const plugin = new TestPlugin(queue);
